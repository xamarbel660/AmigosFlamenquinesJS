...
try {
    ...
    // SELECT * FROM `order_dish` WHERE `id_client_order` = 4
    //SELECT p.*, od.quantity, od.note FROM plate p JOIN order_dish od ON p.id_plate = od.id_plate WHERE od.id_client_order = ?;
    $sql = "SELECT p.*, od.* FROM plate p JOIN order_dish od ON p.id_plate = od.id_plate WHERE od.id_client_order = " . $idPedido . ";";
